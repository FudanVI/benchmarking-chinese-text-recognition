...
class BaseTrainer(object):
  ...
  def train(self, epoch, data_loader, optimizer, current_lr=0.0, 
    ...
    for i, inputs in enumerate(data_loader):
        ...
        rec_pred, att_map, radical_decoder_result = self._forward(input_dict)
        ...
        # print("output[radical_pred] ",output["radical_pred"].shape)
        # print("input_dict['radical_gt'] ", input_dict['radical_gt'].shape)
        ...
        if self.iters % 100 == 0:
          # if True:
          print("radical_loss:", return_dict['losses']["radical_loss"])
          print('[{}]\t'
                'Epoch: [{}][{}/{}]\t'
                'Time {:.3f} ({:.3f})\t'
                'Data {:.3f} ({:.3f})\t'
...
class Trainer(BaseTrainer):
  def _parse_data(self, inputs):
    input_dict = {}
    imgs, labels, lengths,label_ = inputs
