...
class RadicalBranch(nn.Module):
    ...
    def __init__(self, radical_alphabet_len):
        ...
        self.pe_radical = PositionalEncoding(d_model=256, dropout=0.1, max_len=7000)
        self.decoder_radical = Decoder()
        self.generator_radical = Generator(512, self.radical_n_class)

    def forward(self, attention_map_tmp, conv_feature, radical_input, radical_length):
        ...
        # print("attention_map_tmp",attention_map_tmp.shape)
        # print("conv_feature_tmp", conv_feature_tmp.shape)
        # print("conv_feature_tmp",conv_feature_tmp.shape,conv_feature_tmp.device)
        # print(" conv_feature_tmp.unsqueeze(1)", conv_feature_tmp.unsqueeze(1).shape, conv_feature_tmp.shape)
        ...
        # print("char_maps1", char_maps.shape)#[B*L,C,8,8]

        b, l, rl = radical_input.size()
        # print("radical_input",radical_input.shape)
        ...
        # print("postion_embedding_radical",postion_embedding_radical.shape,embedding_radical.shape)
        radical_input_with_pe = torch.cat([embedding_radical, postion_embedding_radical], 2)


        # print("radical_input_with_pe",radical_input_with_pe.shape)
        # print("char_maps", char_maps.shape)
        ...
        return radical_decoder_result
        # print('radical_decoder_result size:', radical_decoder_result.size(), radical_decoder_result.dtype)
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None, align=None):
        ...
        #     print(cnt,l,x)
        #     cnt += 1
        # print("在Multi中，query的尺寸为", query.view(nbatches, -1, self.h, self.d_k).shape)
        # print("在Multi中，key的尺寸为", key.view(nbatches, -1, self.h, self.d_k).shape)
        # print("在Multi中，value的尺寸为", value.view(nbatches, -1, self.h, self.d_k).shape)
        # print("nbatches  h   dk", nbatches,self.h,self.d_k)
        # 1) Do all the linear projections in batch from d_model => h x d_k
        query, key, value = \
            [l(x).view(nbatches, -1, self.h, self.d_k).contiguous().transpose(1, 2)
             for l, x in zip(self.linears, (query, key, value))]
...
if __name__ == '__main__':
    ...
    radical_length = torch.ones((16, 22)).int()
