...
from model.fracPickup import fracPickup
from model.radical_branch import RadicalBranch
    ...
    def forward(self, feats, text_length, text, test=False):
        ...
        if not test:
            ...
            for i in range(num_steps):
                ...
                att_map.append(alpha)
            att_map = torch.stack(att_map, dim=1)
...
class ResNet(nn.Module):
    def __init__(self,c_in):
        ...
        self.block3 = self._make_layer(64, 128, (1,1), 6)
        self.block4 = self._make_layer(128, 256, (1,1), 6)
        self.block5 = self._make_layer(256, 512, (1,1), 3)
...
class ASRN(nn.Module):
    ...
    def __init__(self, imgH, nc, nclass, nh, BidirDecoder=False, CUDA=True):
        ...
        self.radical_branch = RadicalBranch(nclass)
    ...
    def forward(self, input, length, text, text_rev, length_radical=None, radical_input=None, test=False):
        # conv features
        conv = self.cnn(input)
        V = conv
        b, c, h, w = conv.size()
        ...
        if self.BidirDecoder:
            ...
            output, att_map = self.attention(rnn, length, text, test)
            if (not test) and (length_radical is not None) and (radical_input is not None):
                radical = self.radical_branch(att_map, V, radical_input, length_radical)
            else:
                radical = None
            return output, radical

if __name__ == "__main__":
    x = torch.randn(16, 3, 32, 256)
    cnn = ResNet(3)
    output = cnn(x)
    print(output.shape)
