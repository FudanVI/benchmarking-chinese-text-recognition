...
class AttentionRecognitionHead(nn.Module):
  ...
  """
    ...
    def forward(self, x, embed):
        x, targets, lengths, max_lengths = x
        batch_size = x.size(0)
        # Decoder
        ...
        for i in range(max_lengths):
            ...
            output, state, att_prob = self.decoder(x, state, y_prev)

            outputs.append(output)
            att_map.append(att_prob)
        outputs = torch.cat([_.unsqueeze(1) for _ in outputs], 1)
        attmaps = torch.cat([_.unsqueeze(1) for _ in att_map], 1)
        # print("attmaps", attmaps.shape)
        return outputs, attmaps
    ...
    def beam_search(self, x, beam_width, eos):
        ...
        att_map = []
        # print("self.max_len_labels",self.max_len_labels)
        for i in range(self.max_len_labels):

            output, state, att = self.decoder(inflated_encoder_feats, state, y_prev)
...
class DecoderUnit(nn.Module):
    ...
    def get_initial_state(self, embed, tile_times=1):
        ...
        if tile_times != 1:
            state = state.unsqueeze(1).permute(1, 0, 2).repeat(tile_times, 1, 1).permute(1, 0, 2).contiguous().view(-1,
                                                                                                                    self.sDim)
